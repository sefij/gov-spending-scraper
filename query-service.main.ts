...
app.post('/api/top_suppliers', async (req, res, next) => {
    try {
        ...
        const dates = {
            from: DateTime.fromISO(requestPayload.from_date).toISODate(),
            to: DateTime.fromISO(requestPayload.to_date).toISODate()
        }

        let invlidInputMessage

        if (
            !requestPayload.from_date ||
            !requestPayload.to_date ||
            !requestPayload.buyer_name
        ) {
            invlidInputMessage =
                'Invalid input, make sure your input includes the buyer_name and valid dates for from_date and to_date'
        }

        if (
            DateTime.fromISO(requestPayload.from_date) >
            DateTime.fromISO(requestPayload.to_date)
        ) {
            invlidInputMessage = `Invalid input,make sure from_date is before to_date, ${JSON.stringify(
                {
                    from_date: requestPayload.from_date,
                    to_date: requestPayload.to_date
                }
            )}`
        }

        if (invlidInputMessage) {
            res.status(400).json({
                message: invlidInputMessage
            })
            return
        }
        ...
        const result = await knexDb('spend_transactions')
            .where({
                buyer_name: requestPayload.buyer_name
            })
            .whereBetween('transaction_timestamp', [dates.from, dates.to])
            .select(
        ...
        if (!result.length) {
            res.status(404).json({
                message: 'No matching result were found'
            })
        } else {
            res.json({ top_suppliers: result })
        }
    } catch (err) {
        next(err)
    }
